...
public abstract class Node {
	...
	public void generate() {
	    ...
	    final String superClass = type.supers.getFirst().qualifiedName.toString();
	...
	// Code-gen: http://osdir.com/ml/java.objectweb.asm/2008-12/msg00036.html
	...
	public void preAnalyse() {
	    ...
	    if(!hasConstructor){
		// Give the class a default constructor if a constructor is not already declared
		LinkedList<NodeModifier> mods = new LinkedList<NodeModifier>();
		mods.add(new NodeModifier(0, 0, "public"));
		funcDecs.add(new NodeFuncDec(0, 0, mods, "init", new NodeArgs(), null, new NodeFuncBlock(), new NodeTypes(), false));
	...
	@Override
	public void generate() {
	    for(NodeFuncDec dec : funcDecs) dec.generate();
	    for(NodeVarDec dec : varDecs) dec.generate();
	}
	...
	public void generateConstructor(LinkedList<NodeVarDec> varDecs) {
	    generate();
	    // We have to initlialse the class' fields to their default values in the constructor
	    for(NodeVarDec dec : varDecs){
		IExpression expr = null;
		if(dec instanceof NodeVarDecImplicit){
		    expr = ((NodeVarDecImplicit)dec).expr;
		}else expr = ((NodeVarDecExplicit)dec).expr;
		
		if(expr == null){
		    expr = dec.var.type.getDefaultValue();
		}
		
		if(expr != null){
		    ((Node) expr).generate();
		    // Add the statements generated by this expression to the constructor's body.
	...
	public void generate() {
		...
		GenNode.currentFunction.stmts.add(new GenNodeReturn(funcReturnType.getInstructionType()));
	    }else if(singleLineStmt != null){
		((Node) singleLineStmt).generate();
		GenNode.currentFunction.stmts.add(new GenNodeReturn(EnumInstructionOperand.VOID));
	    }else{
		for(IFuncStmt stmt : stmts) ((Node) stmt).generate();
		// Add a return statement if the func is void
		if(funcReturnType != null && funcReturnType.isVoid()) GenNode.currentFunction.stmts.add(new GenNodeReturn(EnumInstructionOperand.VOID));
	    }
	}
	...
	public void generate() {
	    ...
	    for(TypeI type : func.parameters) sb.append(type.toBytecodeName());
	...
	public void generate() {
	    ((Node) expr).generate();
	    GenNode.currentFunction.stmts.add(new GenNodeReturn(exprType.getInstructionType()));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeInt(val));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeLong(val));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeFloat(val));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeDouble(val));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeString(val));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeInt(val ? 1 : 0));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeInt((int)val));
	}
	...
	@Override
	public void generate() {
	    GenNode.currentFunction.stmts.add(new GenNodeNull());
	}
