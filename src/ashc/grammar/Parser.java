...
public class Parser {
	...
	while (true) {
		...
		if (token == null) {
		    break; // we have passed EOF
		}
		tokens.add(token);
	    } catch (final InvalidTokenException e) {
		System.err.printf("Error:%d:%d Invalid token (%s)%n", e.line,
			e.column, e.data);
	    }
	}
    }
	...
	if ((token.type != t) && !silenceErrors) throw new UnexpectedTokenException(
		token, t);
	return token;
    }

    private Token expect(final String tokenData)
	    throws UnexpectedTokenException {
	final Token next = getNext();
	if (!next.data.equals(tokenData) && !silenceErrors) throw new UnexpectedTokenException(
		next, tokenData);
	return next;
    }
	...
	if (!found && !silenceErrors) throw new UnexpectedTokenException(token,
		t);
	return token;
    }
     ...
     *
     */
    private LinkedList<NodeImport> parseImports()
	    throws UnexpectedTokenException {
     ...
     *
     */
    private NodeQualifiedName parseQualifiedName()
	    throws UnexpectedTokenException {
	Token id = expect(TokenType.ID);
	final NodeQualifiedName name = new NodeQualifiedName(id.line,
		id.columnStart);
	name.add(id.data);
	...
	if (getNext().type == TokenType.PACKAGE) {
	    final NodeQualifiedName name = parseQualifiedName();
	    if (name != null) return new NodePackage(name.line, name.column,
		    name);
	}
	rewind();
	return null;
	...
	return (type == TokenType.PUBLIC) || (type == TokenType.PRIVATE)
		|| (type == TokenType.PROTECTED) || (type == TokenType.FINAL)
		|| (type == TokenType.REQUIRED) || (type == TokenType.NATIVE)
		|| (type == TokenType.OVERRIDE) || (type == TokenType.STANDARD)
		|| (type == TokenType.STATIC);
    }

    private LinkedList<NodeModifier> parseMods()
	    throws UnexpectedTokenException {
     ...
     */
    private NodeClassDec parseClassDec(final LinkedList<NodeModifier> mods)
	    throws UnexpectedTokenException {
	...
	if (getNext().type == TokenType.COLON) {
	    types = parseSuperTypes();
	} else {
	    rewind();
	}

	final NodeClassBlock block = parseClassBlock();
	return new NodeClassDec(id.line, id.columnStart, mods, types, args, id,
		block, generics);
    }
     ...
     * func id(args?) (: type)?
     */
    private NodeFuncDec parseFuncDec(final boolean needsBody,
	...
	expect(TokenType.COLON, TokenType.ARROW, TokenType.LAMBDAARROW,
		TokenType.BRACEL);
	rewind();
	...
	if (getNext().type == TokenType.ARROW) {
	...
	Token token = null;
	if (allowSingleLine) {
	    token = expect(TokenType.LAMBDAARROW, TokenType.BRACEL);
	} else {
	    token = expect(TokenType.BRACEL);
	}
	...
	final Token token = expect(TokenType.ID, TokenType.SELF, TokenType.IF,
		TokenType.WHILE, TokenType.FOR, TokenType.VAR, TokenType.CONST,
		TokenType.RETURN);
	switch (token.type) {
	    case RETURN:
		rewind();
		...
		if (stmt instanceof NodeVariable) {
		    final Token assignOp = expect(TokenType.ASSIGNOP,
			    TokenType.COMPOUNDASSIGNOP);
		    return new NodeVarAssign(assignOp.line,
			    assignOp.columnStart, (NodeVariable) stmt,
			    assignOp.data, parseExpression());
		} else return stmt;
	    case IF:
		return parseIfStmt();
	    case WHILE:
	...
	do {
		...
		final NodeExprs exprs = parseCallArgs(TokenType.PARENL,
			TokenType.PARENR);
		prefix = new NodeFuncCall(id.line, id.columnStart, id.data,
			exprs, prefix, generics);
	    } else {
		rewind();
		prefix = new NodeVariable(id.line, id.columnStart, id.data,
			prefix);
    ...
    private NodeExprs parseCallArgs(final TokenType start, final TokenType end)
	    throws UnexpectedTokenException {
    ...
    private IExpression parsePrimaryExpression()
	    throws UnexpectedTokenException {
	Token next = expect(TokenType.NULL, TokenType.ID, TokenType.THIS,
		TokenType.SELF, TokenType.UNARYOP, TokenType.BRACKETL,
		TokenType.PARENL, TokenType.OCTINT, TokenType.HEXINT,
		TokenType.BININT, TokenType.INT, TokenType.LONG,
		TokenType.FLOAT, TokenType.DOUBLE, TokenType.STRING,
	...
	switch (next.type) {
		...
		break;
	    case UNARYOP:
		expr = new NodeUnary(next.line, next.columnStart,
			parsePrimaryExpression(), next.data, true);
		break;
	    case BRACKETL:
		...
		break;
	    case LONG:
		expr = new NodeLong(Long.parseLong(next.data.substring(0,
			next.data.length() - 1)));
		break;
	    case FLOAT:
	...
	if (expr != null) {
	    ...
	    switch (next.type) {
		case BINARYOP:
		    return new NodeBinary(expr, next.data,
    ...
    private LinkedList<NodeTupleExprArg> parseTupleExpr()
	    throws UnexpectedTokenException {
    ...
    private NodeTupleExprArg parseTupleExprArg(final boolean named,
	    final boolean unnamed) throws UnexpectedTokenException {
	final Token next = getNext();
	final NodeTupleExprArg arg = new NodeTupleExprArg(next.line,
		next.columnStart);
	if (next.type == TokenType.ID) {
	    if (getNext().type == TokenType.COLON) {
		arg.name = next.data;
	...
	// Postfix unary expression
	    case UNARYOP:
		return new NodeUnary(next.line, next.columnStart, expr,
			next.data, false);
	    case QUESTIONMARK:
    ...
    private NodeVarDec parseVarDec(final LinkedList<NodeModifier> mods)
	    throws UnexpectedTokenException {
	...
	if (type == TokenType.COLON) {
	    ...
	    varDec = new NodeVarDecExplicit(id.line, id.columnStart, mods,
		    keyword.data, id.data, nodeType, null);
	    if (getNext().type == TokenType.ASSIGNOP) {
		varDec = new NodeVarDecExplicit(id.line, id.columnStart, mods,
			keyword.data, id.data, nodeType, parseExpression());
	    } else {
		rewind();
	    }
	} else {
	    varDec = new NodeVarDecImplicit(id.line, id.columnStart, mods,
		    keyword.data, id.data, parseExpression());
	}
	...
	if (getNext().type == TokenType.LAMBDAARROW) {
		...
		if (next.type == TokenType.GET) {
		    if (getBlock != null) {
			if (setBlock == null) throw new UnexpectedTokenException(
				next, TokenType.SET, TokenType.BRACER);
		    ...
		    if (setBlock != null) {
			if (getBlock == null) throw new UnexpectedTokenException(
				next, TokenType.GET, TokenType.BRACER);
			else throw new UnexpectedTokenException(next,
				TokenType.BRACER);
		    } else {
	...
	if (id.type == TokenType.ID) {
	    ...
	    if (expect(TokenType.COMMA, TokenType.ASSIGNOP, TokenType.PARENR).type == TokenType.ASSIGNOP) {
	...
	rewind();
	if (getNext().type == TokenType.QUESTIONMARK) {
	    type.optional = true;
	} else {
	    rewind();
	}
    ...
    public NodeTupleType parseTupleType(final boolean mustBeNamed,
	    final boolean mustBeUnnamed) throws UnexpectedTokenException {
	final String id = expect(TokenType.ID, TokenType.PRIMITIVE).data;
	final Token next = getNext();
	if (next.type == TokenType.COLON) {
	    if (mustBeUnnamed) throw new UnexpectedTokenException(next,
		    TokenType.COMMA, TokenType.BRACKETR);
	    final NodeTupleType type = new NodeTupleType(line, column,
		    parseType());
	    type.name = id;
	    return type;
	} else if (mustBeNamed) throw new UnexpectedTokenException(next,
		TokenType.COLON);
	rewind(2);
    ...
    private NodeInterfaceDec parseInterfaceDec(
	    final LinkedList<NodeModifier> mods)
	    throws UnexpectedTokenException {
	final Token id = expect(TokenType.ID);
	NodeArgs args = null;
	...
	final NodeClassBlock block = parseClassBlock();
	return new NodeInterfaceDec(id.line, id.columnStart, mods, types, args,
		id, block);
    }

    private NodeTypeDec parseTypeDec() throws UnexpectedTokenException {
	final LinkedList<NodeModifier> mods = parseMods();
	final Token token = expect(TokenType.CLASS, TokenType.ENUM,
		TokenType.INTERFACE);
	switch (token.type) {
	    case CLASS:
    ...
    private LinkedList<NodeTypeDec> parseTypeDecs()
	    throws UnexpectedTokenException {
    ...
    private NodeEnumInstance parseEnumInstance()
	    throws UnexpectedTokenException {
	final Token id = expect(TokenType.ID);
	if (getNext().type == TokenType.PARENL) {
	    rewind();
	    return new NodeEnumInstance(id.line, id.columnStart, id,
		    parseCallArgs(TokenType.PARENL, TokenType.PARENR));
	}
	rewind();
